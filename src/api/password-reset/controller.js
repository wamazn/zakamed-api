...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your ZakemedApi account.<br>
        ...
        &mdash; ZakemedApi Team
      `
      return sendMail({ toEmail: email, subject: 'ZakemedApi - Password Reset', content })
    })
